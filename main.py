...
def main(year: int, month: int):
    ...
    # get the current date and the day before the current date
    # because the updating frequency for the data sets we use are
    ...
    dates = confirm.keys()
    # create new null lists for daily total number of confirmed/death/recovery
    # (add all countries together) cases worldwide
    world_cases = []
    world_death = []
    world_recoveries = []
    for i in dates:
    ...
    # create a new column called 'sum' and assign the latest date of
    # confirmed/deaths cases (the last two) as the sum cases
    ...
    # Since then, we have completed all parts of data visualization work and
    # let's move forward to the prediction part.

    # We use SVM model for predicting classification.
    # The basic logics are choose features from one month
    # (avg confirmed/deaths cases/Case_fatality_ratio per countries),
    # together with other features (Density, Continents, people_fully_vaccinated_per_hundred, 'GNI') as features,
    # and use average increased confirmed cases per day as label to make classifications.
    # According to label, we split the
    ...
    # leave the last date of the month; third, use vaccination rate on the last day
    # as the vaccination rate for the month
    ...
    # find the last date that has records in the month and use the vaccination rate
    # on that date as the rate for the month
    ...
    df_features['label'] = 0
    # we assign a country which has under 1500 average increased confirmed cases
    # in the given month as 0 (impossible to have
    ...
    # 'covid outbreak'; 23 countries that are correctly predicted as
    # 'no covid outbreak'; 6 countries that are incorrectly
...
if __name__ == "__main__":
    # You can set the year and month here
    year = 2021
    month = 12
    main(year, month)
